...
#define NEW_HELPER_FOR_MICRO_SD 55
#define APPEND_HELPER_TO_MICRO_SD 56
#define FINISH_WRITING_HELPER_TO_MICRO_SD 57
...
void check_command(){
  ...
  switch (command[1]) {
    ...
    case GET_KEYS:
      ...
      break;
    case NEW_HELPER_FOR_MICRO_SD:
      myFile = SD.open("h.txt", O_WRITE | O_CREAT | O_TRUNC);
      Serial.write(99);
      Serial.write(NEW_CHALLENGE_FOR_MICRO_SD);
      Serial.write(myFile ? 1 : 0);
      for (int i=0; i < 33; i++)
        Serial.write(0);
      break;
    case APPEND_HELPER_TO_MICRO_SD:
      memcpy(&a, &command[2], 4);
      myFile.println(a);

      Serial.write(99);
      Serial.write(APPEND_HELPER_TO_MICRO_SD);
      Serial.write((uint8_t) (a >> 24) & 0xFF);
      Serial.write((uint8_t) (a >> 16) & 0xFF);
      Serial.write((uint8_t) (a >> 8) & 0xFF);
      Serial.write((uint8_t) a & 0xFF);
      for (int i=0; i < 36-6; i++)
        Serial.write(0);
      break;
    case FINISH_WRITING_HELPER_TO_MICRO_SD:
      myFile.close();

      Serial.write(99);
      Serial.write(FINISH_WRITING_HELPER_TO_MICRO_SD);
      for (int i=0; i < 36-2; i++)
        Serial.write(0);
      break;
    default:
